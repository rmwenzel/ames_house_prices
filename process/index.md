...
First we'll do preliminary processing and cleaning of the original dataset. Later we'll [explore the cleaned data]({{site.baseurl}}/explore/) and [select/engineer features model and predict sale prices]({{site.baseurl}}/model).

Local copies of all datasets are in `house_prices/data`. Custom helper code is in `codes/process.py`.
